--[[
	...
	Round 8+ pool category ("Simple algebra"): one-step linear equations
	with an integer solution. Four equally-likely forms: x + a = b,
	x - a = b, m·x = b, x ÷ m = q.
]]
...
function Algebra.Generate(): RawQuestion
	...
	elseif form == 3 then
		...
		local m = randomInt(2, 12)
		local q = randomInt(2, 20) -- the equation's right-hand side; x = q * m
		local x = m * q
		return {
